...
class foodCollections: ObservableObject {
    ...
    @Published var listOfFood = [food]()
    @Published var listOfPinnedFood = [food]()
    @Published var listOfFoodInGroups = [food]()
    @Published var listOfPinnedFoodInGroups = [food]()
    @Published var addedFoodItems = [food]()
    @Published var selectedItem: food?
    
    private var _resultsOfSearch = [food]()
    ...
    func fillList() {
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            ...
            temp.append(food(table_id: _id, name: _name, prot: _prot, fat: _fat, carbo: _carbo, kkal: _kkal, gi: _gi, index: _index, position: _position))
            _position += 1
        }
