    ...
    func sendEmail() async {
        ...
        if email != [""] {
            do {
                ...
                try emailSender.shared.sendEmail(subject: diaryStatblock.statBlock.formMailSubject(), body: diaryStatblock.statBlock.formMailBoodyMessage(), to: email, xlsxFile: content)
            } catch {
